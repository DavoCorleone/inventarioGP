...
function LoginForm() {
    ...
    const createUser = useMutation(api.auth.createPendingUser);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isRegistering) {
                ...
                setSuccessMsg("Te hemos enviado un enlace mágico a tu correo. Revisa tu bandeja de entrada.");
            }
        } catch (err: any) {
            setError(err.message || "Error ocurrido");
        } finally {
            setLoading(false);
        }
    };
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {!isRegistering && (
                        <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 12, textAlign: "center" }}>
                            Te enviaremos un <b>enlace mágico</b> a tu correo electrónico para acceder sin contraseña.
                        </p>
                    )}
                </form>
